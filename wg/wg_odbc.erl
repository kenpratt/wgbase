...
start_link(StartInterval) ->
    gen_server:start_link(?MODULE, [StartInterval], []).
...
init([StartInterval]) ->
    ...
    case wg_config:get(?WG_CONF, odbc_keepalive_interval) of
        KeepaliveInterval when is_integer(KeepaliveInterval) ->
            timer:send_interval(KeepaliveInterval * 1000, self(), keep_alive);
        undefined ->
            ok;
        _Other ->
            ?Error2("Wrong odbc_keepalive_interval definition '~p' .~n", [_Other])
    end,

    case wg_config:get(?WG_CONF, odbc_server) of
        %% Default pgsql port
